...
- **Round off the content, not the focusable box.** A component that crops an image to a shape (`.ui-avatar`) gives the `<img>` `border-radius: inherit` rather than putting `overflow: hidden` on the container, so a `<button>`/`<a>` version keeps a whole focus ring and edge decorations (a status dot) can sit proud of the box.
...
- **A composite field reads its state off the control it wraps.** Where a component puts a native input inside a shell that owns the border and the ring (`.ui-number`), the shell's states come from `:has()` on that input — `:has(.ui-input:focus-visible)`, `:invalid`, `:read-only`, `:disabled` — and the input's own ring is cleared, so one ring is drawn and the app has no state to mirror onto the wrapper.
- **A visually hidden control keeps its focus ring.** Where a component hides the native input and dresses a sibling as the affordance (`.ui-file`), the input is clipped rather than `display: none`, so it stays in the focus order, and the ring is drawn on the sibling with `input:focus-visible + .part`. State that lives on the hidden input (`:disabled`, `aria-invalid`) reaches the wrapper through `:has()` inside `@supports selector(:has(*))`.
